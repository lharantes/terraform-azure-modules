...
resource "azurerm_mysql_flexible_server" "this" {
  ...
  name                              = lower("azpdy${lookup(local.environment, var.environment, false)}${format("%04d", var.suffix)}")
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  ...
  name                           = each.value.name != null ? each.value.name : "diag-${lower("azpdy${lookup(local.environment, var.environment, false)}${format("%04d", var.suffix)}")}"
