resource "azurerm_postgresql_flexible_server" "this" {
  ...
  name                              = lower("azpdg${lookup(local.environment, var.environment, false)}${format("%04d", var.suffix)}")
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  ...
  name                           = each.value.name != null ? each.value.name : "diag-${lower("azpdg${lookup(local.environment, var.environment, false)}${format("%04d", var.suffix)}")}"
